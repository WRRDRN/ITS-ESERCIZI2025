class Movie:
    def __init__(self, movie_id: str, title: str, director: str, is_rent=False):
...
class Customer:
    ...
    def rent_movie(self, movie: Movie) -> None:
        if not movie.is_rent:
            movie.rent()
            self.rented_movies.append(movie)
        else:
            print(f"Il film '{movie.title}' è già stato affittato")
...
class VideoRentalStore:
    def __init__(self, movies: dict, customers: dict):
        self.movies = movies
        self.customers = customers

    def add_movie(self, movie_id: str, title: str, director: str) -> None:
        if movie_id in self.movies:
            print(f"Il film '{title}' è già presente nel catalogo")
        else:
    ...
    def register_customer(self, customer_id: str, name: str) -> None:
        if customer_id in self.customers:
            print(f"Il cliente è già registrato")
        else:
    ...
    def rent_movie(self, customer_id: str, movie_id: str) -> None:
        if customer_id in self.customers and movie_id in self.movies:
            ...
            print(f"Film o cliente non trovato")
